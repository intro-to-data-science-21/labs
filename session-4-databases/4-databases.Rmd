...
Accessing databhasses in R requires a few packages: 
 ...
 * Some package to interact with the back-end of the remote database such as `RSQLite`(there are other options though)
...
* The arguments to `DBI::dbConnect()` vary from database to database, but the first argument is always the database backend (a way to control your queries).
* For instance, it’s 
...
So how do you query a table in the flights database? 
...
Let us call `dbListTables()` again on our "con" database connection. As you can see, there are several more tables now. If you work with an already existing database, you are likely to encounter several tables. 
...
In short, you can conduct almost all your analysis in R in a way that you are used to without these data having to be stored locally. There are however some differences between ordinary data frames and remote database queries that are worth pointing out. 
