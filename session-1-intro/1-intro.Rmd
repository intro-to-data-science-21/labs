...
First thing to remember - *everything* in R is an object.
...
Some useful operations to learn something about a data frame:
```
head(my_df)
tail(my_df)
dim(my_df)
...
**Exercise 10**: How would you access all elements of the variable `country` using indexing rather than the `$` operator?
```{r, echo = F}
head(mydata[, 1])
head(mydata[, "country"])
```

**Exercise 11**: How would you find the maximum value for number of events using the `$` operator?
```{r, echo = F}
max(mydata$nconflict)
```

**Exercise 12**: Print the country that corresponds to the maximum world population value using the `$` operator and indexing!
```{r, echo = F}
mydata$country[mydata$nconflict == max(mydata$nconflict)]
```

**Exercise 13**: Print out every second element from the variable `country` using indexing methods and the sequence function `seq()`.
```{r, echo = F}
...
## Let's go, learn R! `r emo::ji("backpack")`
